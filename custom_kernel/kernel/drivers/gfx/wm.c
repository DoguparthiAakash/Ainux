...
/* Embedded Font Data to prevent Linking Issues */
static const uint8_t font_8x8_local[95][8] = {
...
static void wm_draw_char_bb(int x, int y, char c, uint32_t color) {
    int idx = c - 32;
    if (idx < 0 || idx >= 95) idx = 0;
    const uint8_t *glyph = font_8x8_local[idx];
    for(int i=0; i<8; i++) {
        uint8_t row = glyph[i];
        for(int j=0; j<8; j++) {
...
/* Boot Menu State */
static int boot_menu_active = 0;
static int boot_selection = 1;

void wm_set_boot_menu(int active, int selection) {
    boot_menu_active = active;
    boot_selection = selection;
    wm_mark_dirty();
}

static void wm_render_boot_menu(void) {
    /* Center Box */
    int box_w = 400;
    int box_h = 300;
    ...
    // wm_fill_rect_bb is solid. Let's use it for now with a dark color
    wm_fill_rect_bb(box_x, box_y, box_w, box_h, 0xAA202020);
    // Draw Header
    wm_fill_rect_bb(box_x, box_y, box_w, 40, 0x444488);
    /* Draw Title */
    // wm_draw_string_bb(box_x + 120, box_y + 12, "AINUX BOOT MENU", 0xFFFFFF);
}
