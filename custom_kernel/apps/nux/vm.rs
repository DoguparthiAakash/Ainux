use crate::fs::vfs::{FileHandle, Inode};
use alloc::collections::BTreeMap;
use alloc::string::String;
use alloc::sync::Arc;
use alloc::vec::Vec;
use core::cell::UnsafeCell;
use core::sync::atomic::{AtomicBool, Ordering}; // Import VFS types
            ...
            match op {
                    ...
                    if ms > 0 {
                        #[cfg(target_os = "none")]
                        {
                            ...
                            if ticks > 0 {
                                ...
                                unsafe {
                                    crate::process::scheduler::set_current_sleep(
                                        current + ticks as u64,
                                    );
                                }
                ...
                OP_FILE_OPEN => {
                    ...
                    // ... (lookup success block above)
                        ...
                        if let Ok(inode) =
                            crate::fs::vfs::root().create(&path, crate::fs::vfs::FileType::File)
                        {
                            if let Ok(handle) = inode.open(0) {
                                let fd = shared.next_fd;
                                shared.next_fd += 1;
                ...
                OP_FILE_READ => {
                    ...
                    if let Some(handle) = shared.files.get(&fd) {
                        ...
                        if let Ok(count) = handle.read(&mut temp_buf, 0) {
                            // Offset 0 for now
                            // Copy back to memory
                            for i in 0..count {
                ...
                OP_FILE_WRITE => {
                    ...
                    if let Some(handle) = shared.files.get(&fd) {
                        ...
                        if let Ok(count) = handle.write(&temp_buf, 0) {
                            // Offset 0
                            drop(shared);
                            self.push(count as i64);
                        } else {
                ...
                OP_FILE_MKDIR => {
                    ...
                    if let Ok(_) =
                        crate::fs::vfs::root().create(&path, crate::fs::vfs::FileType::Directory)
                    {
                        self.push(1);
                    } else {
                        self.push(0);
                ...
                OP_DM_SET => {
                    ...
                    while addr < mem.len() && mem[addr] != 0 {
                        val_str.push(mem[addr] as char);
                        addr += 1;
                    }
                    drop(shared);

                    crate::api::datamanager::DATA_MANAGER
                        .lock()
                        .set(&key, &val_str);
                    self.push(0); // Void
                }
                ...
                OP_SEC_LOGIN => {
                    ...
                    while addr < mem.len() && mem[addr] != 0 {
                        user.push(mem[addr] as char);
                        addr += 1;
                    }

                    addr = pass_ptr as usize;
                    while addr < mem.len() && mem[addr] != 0 {
                        pass.push(mem[addr] as char);
                        addr += 1;
                    }
                    drop(shared);

                    let success = crate::security::user::USER_MANAGER
                        .lock()
                ...
                OP_SEC_WHOAMI => {
                    let name = crate::security::user::USER_MANAGER
                        .lock()
                        .get_current_user();
                ...
                _ => {
                    kprintln!("Unknown Opcode: {:02X}", op);
                }
            }
        }
    }
