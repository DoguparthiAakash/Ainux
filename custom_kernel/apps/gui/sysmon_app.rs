...
pub fn sysmon_main() {
    ...
    loop {
        ...
        if t.saturating_sub(app.last_refresh) >= 30 {
            // Sample tick delta
            let delta = t.saturating_sub(app.last_ticks);
            app.tick_history[app.tick_history_idx] = delta;
            ...
            app.proc_history[app.tick_history_idx] = proc_count;

            app.tick_history_idx = (app.tick_history_idx + 1) % 60;
            app.last_ticks = t;
            app.last_refresh = t;
            app.frame += 1;
        ...
        if app.needs_redraw {
            ...
            SysMonApp::fill(buf, w, h, 0, 0, w as i32, h as i32, 0xFF1E1E2E);

            // Title
            SysMonApp::fill(buf, w, h, 0, 0, w as i32, 22, 0xFF313244);
            SysMonApp::text(buf, w, h, 4, 4, "System Monitor", 0xFFCBA6F7);

            // Animated dots to show it's live
            let dot_str = match app.frame % 4 {
                0 => "Live ●   ",
                1 => "Live  ●  ",
                2 => "Live   ● ",
                _ => "Live    ●",
            };
            SysMonApp::text(buf, w, h, w as i32 - 80, 4, dot_str, 0xFFA6E3A1);
            ...
            for (i, s) in stats.iter().enumerate() {
                SysMonApp::fill(buf, w, h, 4, 26 + i as i32 * 18, w as i32 - 8, 16, 0xFF24273A);
                SysMonApp::text(buf, w, h, 8, 28 + i as i32 * 18, s, 0xFFCDD6F4);
            }
            ...
            SysMonApp::fill(buf, w, h, 4, 62, w as i32 - 8, 16, 0xFF24273A);
            SysMonApp::text(buf, w, h, 8, 64, &mem_str, 0xFFCDD6F4);
            ...
            let bar_h  = 12i32;
            SysMonApp::fill(buf, w, h, bar_x, bar_y, bar_w, bar_h, 0xFF45475A);
            if total_kb > 0 {
                ...
                SysMonApp::fill(buf, w, h, bar_x, bar_y, fill_w.max(1), bar_h, col);
            }

            // Tick activity graph
            let graph_y = 102i32;
            let graph_h = (h as i32 - graph_y - 28) / 2;
            let max_tick = *app.tick_history.iter().max().unwrap_or(&1);
            SysMonApp::draw_bar_graph(buf, w, h, 4, graph_y, w as i32 - 8, graph_h,
                &app.tick_history, max_tick.max(1), 0xFF89DCEB, "Tick Activity");

            // Process count graph
            let proc_graph_y = graph_y + graph_h + 6;
            let proc_graph_h = h as i32 - proc_graph_y - 22;
            let max_proc = *app.proc_history.iter().max().unwrap_or(&1);
            let proc_u64: Vec<u64> = app.proc_history.iter().map(|&v| v as u64).collect();
            SysMonApp::draw_bar_graph(buf, w, h, 4, proc_graph_y, w as i32 - 8, proc_graph_h,
                &proc_u64, max_proc.max(1) as u64, 0xFFCBA6F7, "Process Count");

            // Status bar
            SysMonApp::fill(buf, w, h, 0, h as i32 - 18, w as i32, 18, 0xFF313244);
            let proc_count = app.proc_history[(app.tick_history_idx + 59) % 60];
            let status = alloc::format!("{} processes running", proc_count);
            SysMonApp::text(buf, w, h, 4, h as i32 - 15, &status, 0xFF6C7086);
