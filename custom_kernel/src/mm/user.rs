...
// --- Auto Allocator for Isolated Space (Phase 30c) ---
use spin::Mutex;
pub static USER_HEAP: Mutex<UserHeap> = Mutex::new(UserHeap::new());
...
impl UserHeap {
    ...
    pub fn allocate(&mut self, len: usize) -> Result<u64, UserError> {
        // Simple Bump Allocator for "Clear Linux" style speed
        let align = 16;
        let start = (self.base + self.used as u64 + (align - 1)) & !(align - 1);
        let new_used = (start - self.base) as usize + len;

        if new_used > self.size {
            return Err(UserError::BufferTooLarge);
        }
