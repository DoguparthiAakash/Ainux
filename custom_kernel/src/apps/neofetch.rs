// =============================================================================
// Ainux Neofetch — System Information Display
// Displays hardware info with ASCII art logo, inspired by Linux neofetch.
// =============================================================================
...
/// Reports vendor name + raw PCI IDs so any unknown device is identified.
    ...
    for bus in 0u16..256 {
        for slot in 0u8..32 {
            ...
            if class_id == 0x03 { // Display controller
                ...
                // Read PCI subsystem IDs (offset 0x2C) for more specific identification
                let subsys_reg = pci_config_read(bus as u8, slot, 0, 0x2C);
                let subsys_vendor = (subsys_reg & 0xFFFF) as u16;
                let subsys_device = (subsys_reg >> 16) as u16;
...
/// Detect total RAM in MB from PMM stats
...
/// Get framebuffer resolution string
fn detect_resolution() -> String {
    // These are set during PMM init from multiboot info
    // We read them from the video driver
...
/// Main neofetch display
pub fn cmd_neofetch() {
    // Copy strings out of the lock guard into owned Strings
    ...
    // Ainux ASCII art logo (left side) paired with info (right side)
    let logo: [&str; 16] = [
        "                                ",  // 0
        " ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▒▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓ ",  // 1
        " ▓▓▓▓▓▓▓▓▓▓▓▓▓▒░░▓▓▓▓▓▓▓▓▓▓▓▓▓▓ ",  // 2
    ...
    video::put_str(logo[5]); video::put_str("Shell:      ainux-sh (native)\n");
