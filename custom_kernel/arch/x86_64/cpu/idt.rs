...
extern "C" fn rust_page_fault_handler(ctx: *const ExceptionContext) {
    ...
    unsafe { 
        ...
        // If the fault happened in user mode (bit 2), terminate the process instead of hanging the OS
        if c.err & 4 != 0 {
            print_serial("User mode page fault! Terminating process.\n");
            crate::process::scheduler::exit_current_task(139); // SIGSEGV
            return;
        }

        // Hang on kernel mode fault
        loop {}
    }
}
...
fn timer_handler_addr() -> u64 {
    crate::microkernel::scheduler::switch_kernel_context as u64
}
