...
int initrd_init_memory(uint64_t address, uint64_t size) {
    if (address == 0 || size == 0) {
        kprint("[InitRD] Invalid module address/size!\n");
        return -1;
    }
    
    initrd_capacity = 32 * 1024 * 1024;
    initrd_start = (uint8_t *)kmalloc(initrd_capacity);
    
    if (!initrd_start) {
        kprint("[InitRD] Failed to allocate RAM buffer!\n");
        initrd_start = (uint8_t *)address;
        initrd_size = size;
        ...
        mem_cpy(initrd_start, (void*)address, copy_size);
    ...
    while (ptr < initrd_start + initrd_size) {
        ...
        if (header->typeflag != 'X') {
            ...
            if (str_cmp(hname, name) == 0) {
                struct initrd_file *match = (struct initrd_file *)kmalloc(sizeof(struct initrd_file));
                if (match) {
                    /* Manual copy to avoid str_cpy issues */
                    int i = 0;
                    while(hname[i] && i < 255) {
                        match->name[i] = hname[i];
                        i++;
                    }
                    match->name[i] = 0;
...
static uint64_t initrd_vfs_read(vfs_node_t *node, uint64_t offset, uint64_t size, uint8_t *buffer) {
    ...
    struct initrd_file *file = (struct initrd_file *)node->device;
    
    if (offset > file->size) return 0;
    if (offset + size > file->size) size = file->size - offset;
    
    mem_cpy(buffer, (uint8_t*)file->data + offset, size);
    return size;
}
...
static vfs_node_t *initrd_vfs_finddir(vfs_node_t *node, const char *name) {
    ...
    if (node->device) {
        struct initrd_file *parent = (struct initrd_file *)node->device;
        str_cpy(path, parent->name, 200);
        
        /* Ensure trailing slash on parent if missing (for directory concatenation) */
    ...
    struct initrd_file *file = initrd_find_file(path);
    
    /* Retry with trailing slash */
    if (!file) {
        size_t len = str_len(path);
        if (len < 254 && path[len-1] != '/') {
            path[len] = '/';
            path[len+1] = 0;
            file = initrd_find_file(path);
        }
    }
    ...
    if (file) {
        ...
        /* Manual zero */
        uint8_t *p = (uint8_t *)new_node;
        for(size_t k=0; k<sizeof(vfs_node_t); k++) p[k] = 0;
        
        /* Manual name copy */
        size_t k = 0;
        while(name[k] && k < 127) {
            new_node->name[k] = name[k];
            k++;
        }
        new_node->name[k] = 0;
        
        new_node->length = file->size;
        new_node->device = file; 
        
        new_node->flags = VFS_FILE;
        if (path[str_len(path)-1] == '/') new_node->flags = VFS_DIRECTORY;
        
        new_node->ops = node->ops;
        
        return new_node;
    } else {
        /* kprint("[InitRD] Fail: "); kprint(path); kprint("\n"); */
    }
    return 0;
}

static struct w_dirent *initrd_vfs_readdir(vfs_node_t *node, uint32_t index) {
    (void)node; (void)index;
    /* TODO: Implement readdir if needed for ls /initrd */
    return 0; 
}

/* Global Ops struct */
static vfs_fs_ops_t initrd_fs_ops = {
    .read = initrd_vfs_read,
    .write = 0, // Read-only for now via VFS or implement write wrapper
    .open = 0,
    .close = 0,
    .readdir = initrd_vfs_readdir,
    .finddir = initrd_vfs_finddir
};

vfs_node_t *initrd_mount_vfs(void) {
    vfs_node_t *root = (vfs_node_t *)kmalloc(sizeof(vfs_node_t));
    mem_set(root, 0, sizeof(vfs_node_t));
    str_cpy(root->name, "initrd", 128);
    root->flags = VFS_DIRECTORY;
    root->ops = &initrd_fs_ops;
    return root;
}
