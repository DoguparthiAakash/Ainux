...
void *asm_compile(const char *source) {
    ...
    while (*ptr) {
        ...
        if (is_digit(*ptr)) {
            has_imm = 1;
            /* Use op2_imm variable for generic imm storage, or add op1_imm */
            /* Let's reuse op2_imm effectively as 'imm_val' */
    ...
    kprint("[ASM] Generated Code (Hex):\n");
    for (int i = 0; i < ip; i++) {
        char buf[8];
        char hex[] = "0123456789ABCDEF";
        buf[0] = hex[(code[i] >> 4) & 0xF];
        buf[1] = hex[code[i] & 0xF];
        buf[2] = ' ';
        buf[3] = 0;
        kprint(buf);
    }
    kprint("\n[ASM] Compilation success. Size: ");
    printf("%d", ip);
    kprint(" bytes.\n");
